...
@section('content')
    ...
    <div class="container py-5">
        <div class="card border-primary shadow-sm mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="{{ $isEdit ? route('showcase.update', $showcase) : route('showcase.store') }}"
                                    ...
                                    @for ($i = 0; $i < $rowCount; $i++)
                                        ...
                                        <tr class="showcase-row">
                                                ...
                                                <select name="items[{{ $i }}][recipe_id]"
                                                    ...
                                                    @foreach ($recipes as $rec)
                                                        <option value="{{ $rec->id }}"
                                                            ...
                                                            @selected(old("items.$i.recipe_id", $item->recipe_id ?? '') == $rec->id)>{{ $rec->recipe_name }}</option>
                                                    @endforeach
                                                </select>
                                                <input type="hidden" name="items[{{ $i }}][department_id]"
                                                    class="dept-id-field"
                                            ...
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="text" name="items[{{ $i }}][price]"
                                                        class="form-control form-control-sm text-end price-field" readonly
                                            ...
                                            <td><input type="number" name="items[{{ $i }}][quantity]"
                                                    ...
                                                    value="{{ old("items.$i.quantity", $item->quantity ?? 0) }}" required>
                                            </td>
                                            <td><input type="number" name="items[{{ $i }}][sold]"
                                                    class="form-control form-control-sm text-center sold-field"
                                                    value="{{ old("items.$i.sold", $item->sold ?? 0) }}" required></td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function recalcRow(row) {
                ...
                deptCell.textContent = opt.dataset.deptName || '';
                deptInput.value = opt.dataset.deptId || '';
                priceIn.value = parseFloat(opt.dataset.price || 0).toFixed(2);
                unitSpan.textContent = (opt.dataset.sellMode === 'kg') ? '€/kg' : '€/pc';

                const qty = +row.querySelector('.qty-field').value || 0;
                const sold = +row.querySelector('.sold-field').value || 0;

                row.querySelector('.potential-field').value = (priceIn.value * qty).toFixed(2);
                row.querySelector('.revenue-field').value = (priceIn.value * sold).toFixed(2);
